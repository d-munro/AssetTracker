...
class DataManager:
    ...
    def __init__(self, all_entries):
        ...
        if (all_entries is not None):
            ...
            self._all_entries = self._all_entries.sort_values(
                ["Ticker", "Date", "Time"])
    ...
    def _check_column_validity(self):
        ...
        if len(required_columns_remaining) > 0:
            raise ValueError(
                "The spreadsheet is missing several required columns")
    ...
    def _generate_percent_change(self):
        ...
        if "Percent Change" in self._all_entries:
            return
        self._all_entries["Percent Change"] = self._all_entries["Price"].pct_change(
        ) * 100
        ...
        false_percentages = self._all_entries["Ticker"].drop_duplicates(
        ).index.array
    ...
    def delete_entries(self, tickers):
        ...
        for ticker in tickers:
            self._visible_entries = self._visible_entries.loc[~(
                self._visible_entries["Ticker"] == ticker)]
    ...
    def load_entries(self, tickers):
        ...
        for ticker in tickers:
            try:
                ...
                self._visible_entries = pd.concat(
                    [new_entry, self._visible_entries])
            except ValueError:
                message = "".join(ticker).join(
                    " is not currently loaded. Loading entries cancelled")
                raise message
        self._visible_entries = self._visible_entries.sort_values(
            ["Ticker", "Date", "Time"])
    ...
    def get_visible_entries(self, tickers, starting_date=None, ending_date=None):
        ...
        if len(entries_df) == 0:
            raise UserWarning(
                "Warning: No entries for the entered assets could be found")
        return entries_df
...
class Request:
    ...
    # set of all requests acting within a timeframe
    _TIME_REQUESTS: Final = {
        PLOT_ASSETS
    }

    def __init__(self, request, assets=None, start=None, end=None):
        """
        Attributes:
            ...
            start (str) - The starting date for the request in the format (yyyy-mm-dd)
            end (str) - The ending date for the request in the format (yyyy-mm-dd)
