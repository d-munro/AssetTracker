...
class IO:
    ...
    # Set to True when testing Program, set to False for actual deploy
    _TEST_MODE: Final = False
    ...
    def get_file_extension(self, file_path):
        ...
        if not extension_found:
            raise ValueError(
                "The file at '{}' does not have an extension".format(file_path))
    ...
    def load(self):
        ...
        file_loaded = False
        response = self.get_yes_no_response(
            "Would you like to load an excel file? (Yes/No)\n")
        if response.lower() == "yes":
            while not file_loaded:
                try:
                    file_path = input(
                        "Enter the path to the file you wish to load:\n")
                    ...
                    print("{}".format(e))
                    response = self.get_yes_no_response(
                        "Would you like to try loading a different file? (Yes/No)\n")
                    if response == "no":
                        self.load_default_file()
                        file_loaded = True
        ...
        while not user_num == manager.Request.QUIT:
            try:
                ...
                if not user_num in standalone_requests:
                    assets = input(
                        "Enter the name of the asset(s) (case-sensitive), separating them by spaces:\n")
                    assets_list = assets.split(" ")
                request = manager.Request(
                    user_num, assets=assets_list)
